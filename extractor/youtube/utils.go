...
func IsAudioSeparated(video *youtube.Video, format *youtube.Format) bool {
	/*
	If the selected format contains no audio channels but we have an audio-only format available
	*/
	if format.AudioChannels == 0 && len(video.Formats.Type("audio")) > 0 {
		return true
	}
	return false
}
